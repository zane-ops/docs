...
- **Authentication**: GitHub OAuth with sessions stored in Redis
...
1. Navigate to [GitHub Developer Settings](https://github.com/settings/apps) and create a new OAuth application.

2. Configure the application with these values:
   - Application name: `<your-app-name>`
   - Homepage URL: `http://guestbook.127-0-0-1.sslip.io` 
   - Authorization callback URL: `http://guestbook.127-0-0-1.sslip.io/api/auth/callback/github`
    <Aside type='note'>
    For production deployments, replace `guestbook.127-0-0-1.sslip.io` with your actual domain name.
    </Aside>

3. After creating the application, GitHub will provide a `Client ID` and `Client Secret`. Add these credentials to your project's `.env` file:
...
2. Configure the authentication environment variables:
    Copy your GitHub OAuth credentials from the [earlier step](#setting-up-github-oauth)
    ```shell
    GITHUB_CLIENT_ID="<your_client_id>" 
    GITHUB_CLIENT_SECRET="<your_client_secret>"
    GITHUB_REDIRECT_URI="http://guestbook.127-0-0-1.sslip.io/api/auth/callback/github"
...
See a production version of this application at: https://guestbook.fredkiss.dev