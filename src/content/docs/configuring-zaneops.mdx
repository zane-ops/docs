---
title: Configuring DNS Records for ZaneOps
...
```txt
@     IN  A    <your-server-ip>       # ROOT_DOMAIN
...
![example DNS records on cloudflare](/images/dns-records.png)
...
<img className="block dark:hidden" src="/images/custom-urls-light.png" alt="service settings page on the custom urls section" />
<img className="!hidden dark:!block" src="/images/custom-urls-dark.png" alt="service settings page on the custom urls section" />
...
But in some cases where you have another proxy on top on your server (ex: [Cloudflare tunnel](https://developers.cloudflare.com/cloudflare-one/connections/connect-networks/), [Cloudflare proxy](https://developers.cloudflare.com/dns/proxy-status/)),
 which handle SSL for you, you might want to disable SSL on your server. 
...
2. Add the records on the public hostname tab on your tunnel instance, all forwarded to http://127.0.0.1:80 :  
   ![Cloudflare tunnel public hostnames](/images/cf-tunnel.png)
