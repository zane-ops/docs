...
By default when creating a service from a GitHub app, auto-deploy on Git push is enabled. 

- You can choose to disable it if you don't want it
- You can choose to automatically cleanup the deploy queue (enabled by default), this way when multiple git pushes are made in quick succession, only the 
  latest one is considered
- You can also add **watch paths**, if you want to only trigger new deployments if certains folders or files have been changed. 
   They are specified as a [Glob pattern](https://en.wikipedia.org/wiki/Glob_(programming)) (Ex: `frontend/**`, `{packages/trpc,packages/db,apps/web}/**`)

<img src={`${ASSETS_SERVER_DOMAIN}/images/auto-deploy-options.png`} alt="Service Auto-deploy options" />

New deployments made by git push will show in the app with the label `git push`:
