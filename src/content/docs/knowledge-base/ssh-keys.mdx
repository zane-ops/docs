...
To access your server via the web UI of ZaneOps, you will need to use SSH keys.
Each Key is associated to a user on your server.
...
2. Once in there, you can create a new Key with a **valid user on your server**, and a name for the key.
...
3. After clicking _"Add new Key"_, ZaneOps will generate a public/private key pair,
    you can use the public key to your server.
...
5. Copy the public SSH key and add it at the end the file in a new line in `$HOME/.ssh/authorized_keys` 
...
5. You can now use the key to SSH to your server from ZaneOps:
...
1. Cannot connect to the PORT **22**: 

    ```shell
    Failed to connect to port 22: Connection refused
    ``` 

    You will need to modify the SSH port to **22** to have access via the web UI:
    ```shell
    # /etc/ssh/sshd_condig
    #... other options
    Port 22
...
1. Public key authentication is disabled:
