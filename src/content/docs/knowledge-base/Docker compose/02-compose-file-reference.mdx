...
# ZaneOps-specific: inline config files defined by content
configs:
  my_config:
    content: |
      server { listen 80; }
...
All variables are referenced with **`${VAR_NAME}`** syntax (curly braces required). Variables referenced without curly braces (`$VAR_NAME`) are left as-is and not expanded.
...
Generates a stable, environment-scoped hostname for inter-service communication. This is the recommended way to reference other services in your stack.
...
Use this only for cross-project or cross-environment communication. Prefer `network_alias` in all other cases.
...
| Property | Reason |
| --- | --- |
| `ports` | Replaced by routing labels |
| `expose` | Not meaningful in Swarm mode |
| `restart` | Use `deploy.restart_policy` instead |
| `build` | ZaneOps requires pre-built images |

Setting `deploy.replicas: 0` is supported and will pause the service (its status becomes `SLEEPING`).
...
ZaneOps extends the Docker Compose `configs` spec with a `content` key that lets you define config file content inline, with no external file needed.
...
Docker configs are immutable. ZaneOps handles this transparently: each config is deployed with a version suffix (`nginx_config_v1`, `nginx_config_v2`, …). The version increments automatically when the `content` changes. Old versions are cleaned up after deployment.
...
The variable must be declared in `x-zane-env` first. A `${VAR}` reference to an undefined variable is left as-is.
